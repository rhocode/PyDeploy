...
class Gittle(object):
    ...
    def _setup_fetched_refs(self, refs, origin, bare):
        ...
        # Base of new refs
        heads_base = str.encode('refs/remotes/' + origin)
        if bare:
    ...
    def _refs_by_pattern(self, pattern):
        ...
        def item_filter(key_value):
            ...
            key, value = key_value
            return key.startswith(str.encode(pattern))
    ...
    def switch_branch(self, branch_name, tracking=None, create=None):
        ...
        myhash = self.branches[branch_name]
        if self.is_working:
            # Remove all files
            self.clean_working()

            # Add files for the current branch
            self.checkout_all(myhash)
    ...
    def clean(self, force=None, directories=None):
        ...
        windowsString = os.path.curdir + '\\\\' + self.path + '\\\\'
        untracked_files = set(map(lambda x: windowsString + x, untracked_files))
